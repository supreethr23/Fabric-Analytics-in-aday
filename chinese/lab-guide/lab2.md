...
今天，您将学习 Microsoft Fabric 的各种主要功能。这是一个介绍性研讨会，旨在向您介绍 Fabric
中提供的各种产品体验和项目。在本次研讨会结束时，您将学习如何使用湖屋、数据流 Gen2、数据管道和 DirectLake 等。
...
1. 打开**浏览器**并导航到 [Microsoft Power BI 门户](https://app.powerbi.com/)。系统会将您导航到登录页面。

   - **注意：** 如果您正在使用实验室环境，您可能会自动登录。

   - **注意：** 如果您未在使用实验室环境，并且已经有 Power BI 帐户，您可能希望在隐私/无痕模式下使用浏览器。
...
11. 系统会将您导航到 **Microsoft** **Fabric 主页**。
...
新工作区已创建，您将能够导航到此工作区。我们将来自不同数据源的数据引入湖屋中，并使用湖屋中的数据来生成模型并生成报表。第一步是创建湖屋。

## 任务 3：创建湖屋

1. 在新创建的工作区 **FAIAD_Username** 中，在左侧导航窗格中找到 **+ 新建项 (1)** 按钮。在此处，您可以开始在工作区中创建新项目。
...
3. "新建湖屋"对话框随即显示。在"名称"文本框中输入 **lh_FAIAD**。
   ...
   - **注意**：**湖屋架构**的预览功能非常有意思，您应该意识到这一点。因为它处于**预览版**状态，因此我们将忽略它，以便不会对实验室环境产生负面影响。该功能正式发布后，我们会将其引入本实验室中。

4. 选择**创建**

   ![](../media/lab-02/image17.png)

   湖屋很快就会创建完毕，然后系统会将您导航到湖屋界面。在左侧面板中，请注意工作区下方有一个湖屋图标。您可以随时单击此图标轻松导航到湖屋。
...
1. 选择屏幕左侧的"工作负载"图标。将打开一个包含 Fabric 体验列表的对话框。体验列表包括 Power BI、Data Factory、Industry Solutions、Real-Time Intelligence、Data Engineering、Data Science 和 Data Warehouse。让我们来探索吧。
...
3. 系统会将您定向到 Data Factory 主页。下面是其各部分的详细说明，旨在指导您逐步有效地使用 Data Factory。数据流 Gen2 是新一代数据流。
   ...
   Data Factory
   是一种工具，可帮助您管理和组织来自不同源的数据。它允许您收集、准备和转换数据，以便有效地使用数据。无论您是初学者还是专家，Data Factory 都能提供用于更轻松、更高效地转换数据的工具。
   ...
   **项目类型：**
   ...
   f) **镜像：** 用于创建数据库的镜像版本以进行备份、测试或只读访问的功能。

   g) **变量库（预览版）：** 包含变量及其默认值的列表。还可以包含保存备选值的其他值集
...
3. 单击屏幕左上角的"返回到工作负荷"。此操作会将您转到主工作负荷页面，您可以在该页面中探索其他工具或部分。

   ![](../media/lab-02/image22.png)

## 任务 6：Real-Time Intelligence 体验

1. 从**工作负荷**页面中，单击 **Real-Time Intelligence** 以继续。     ![](../media/lab-02/image24.png)
...
2. 系统会将您定向到 Real-Time Intelligence 主页。下面是其各部分的详细概述，旨在帮助您有效地逐步使用 Real-Time Intelligence。
   ...
   a. **探索 Real-Time Intelligence示例：** 单击"打开"按钮以使用示例探索实时数据
   分析。
   ...
   f. **探索示例激活器：** 单击"打开"按钮以使用示例激活器并了解 Real-Time Intelligence 的特性和功能。

   g. **激活器入门：** 单击"打开"按钮以开始使用激活器概念并开始有效地使用该工具。
...
2. 系统会将您定向到 **Data Engineering** 主页。下面是其各部分的详细概述，旨在帮助您有效地逐步使用 **Data Engineering**。
   ...
   j. **为 GraphQL 创建您的第一个 API：** 单击"打开"按钮以了解如何为 GraphQL 创建 API。
...
2. 系统会将您定向到 Data Warehouse 主页。下面是其各部分的详细概述，旨在帮助您有效地逐步使用 Data Warehouse。
   ...
   **项目类型：**
   ...
   g. **镜像的 Azure Cosmos DB：** 用于镜像 Azure Cosmos DB。

   h. **镜像的 Azure Database for PostgreSQL (预览版)：** 用于镜像您的现有 Azure
   Database for PostgreSQL 
   
   i. **镜像的 Azure SQL 托管数据库：** 用于镜像 Azure SQL 托管数据库以实现高可用性和灾难恢复。

   j. **镜像的数据库 (预览版)：** 用于复制数据库以实现高可用性和灾难恢复。
...
2. 系统会将您定向到数据库主页。下面是其各部分的详细概述，旨在帮助您有效地使用 数据库。
   ...
   a. **SQL 数据库 (预览版)：** Fabric 中的 SQL 数据库是数据库工作负荷的一部分，可以从 Fabric 中的其他项目访问数据。您的 SQL 数据库数据也在 OneLake 中以可查询格式保持最新，因此您可以使用 Fabric 中的所有不同服务，例如使用 Spark 运行分析、执行笔记本、数据工程、通过 Power BI 报表实现可视化等。
   ...
   b. **数据库概念：** 解释有关交易数据库的常见术语和概念，以便您可以熟悉如何使用 SQL 数据库
