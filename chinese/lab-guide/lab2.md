...
您当前位于**我的工作区**中。若要使用 Fabric
项目，您需要试用许可证和分配有 Fabric 许可证的工作区。让我们开始设置。
...
4. 在**名称**字段中，输入 FAIAD_UserID（在"环境"选项卡中找到）  **注意：** 工作区名称必须唯一。确保"名称"字段下方显示带有"此名称可用" 的绿色复选标记。
...
g.
**将笔记本用于您的湖屋：** 单击"打开"按钮以了解如何将笔记本用于您的湖屋。

h.
**将数据集用于您的湖屋：** 单击"打开"按钮以了解如何将数据集用于您的湖屋。
...
1. 从**工作负荷**页面中，单击 **Data Science** 以继续。     ![](../media/lab-02/image28.png)
...
h. **镜像的 Azure Database for PostgreSQL (预览版)：** 用于镜像您的现有
Azure
